    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(310px, 1fr))', gap: '1.25rem', alignItems: 'stretch' }}>
        {officeList.map((off, idx) => {
          if (!off) return null;
          return (
            <div key={off.id || idx} className="glass-card" style={{ padding: '1.4rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: '1rem', border: '1px solid var(--border-color)', borderRadius: '12px', background: 'var(--bg-surface-elevated)', height: '100%' }}>
              <div>
                {/* Row 1: All Badges & Action Controls */}
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', display: 'grid', gap: '0.5rem', lineHeight: 1.45 }}>
                  ...
                  {off.phone && (
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                      ...
                      <strong style={{ color: 'var(--text-primary)' }}>{off.phone}</strong>
                    </div>
                  )}
                  {off.email && (
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                      <span style={{ color: 'var(--text-muted)', flexShrink: 0, marginTop: '2px' }}>✉️</span>
                      <span style={{ wordBreak: 'break-all', color: 'var(--text-secondary)' }}>{off.email}</span>
                    </div>
                  )}
                </div>
              </div>
