...
const generatePdfBlob = (cand) => {
  ...
  const field = (label, value) => {
    ...
    const lines = doc.splitTextToSize(String(value ?? '-'), colW - 55);
    addText(lines, margin + 55, y);
    y += Math.max(6, lines.length * 5);
  };

  // 1. Personal
  section('1. PERSONAL DETAILS / ЛИЧНЫЕ ДАННЫЕ');
  ...
  if (seaService.length === 0) {
    ...
    seaService.forEach((s, idx) => {
      ...
      doc.setTextColor(30, 30, 30);
      doc.setFontSize(8);
      const row = [s.vesselName, s.vesselType, `${s.dwtGrt || '-'}/${s.engineBhp || '-'}`, s.rankHeld, s.manningCompany, `${s.dateFrom || '-'} — ${s.dateTo || '-'}`];
      let rx = tableX + 1;
  ...
  if (cand.notes) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(60, 60, 60);
    const noteLines = doc.splitTextToSize(cand.notes, colW);
